<template>
	<div class="home-content">
		...
		<div class="statis-box">
			<div id="statis1" class="statis1 animate__animated" v-if="isAuth('jingsaixinxi','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
					<div class="num">{{jingsaixinxiCount}}</div>
					<div class="name">竞赛信息总数</div>
				</div>
			</div>
			<div id="statis2" class="statis2 animate__animated" v-if="isAuth('jingsaibaoming','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-chujia3"></span>
				</div>
				<div class="right">
					<div class="num">{{jingsaibaomingCount}}</div>
					<div class="name">竞赛报名总数</div>
				</div>
			</div>
			<div id="statis3" class="statis3 animate__animated" v-if="isAuth('chengjixinxi','首页总数')">
				<div class="left">
		...
		<!-- 3 -->
		<div class="type3">
			<div id="jingsaibaomingChart1" class="echarts1 animate__animated" v-if="isAuth('jingsaibaoming','首页统计')"></div>
			<div id="jingsaibaomingChart2" class="echarts2 animate__animated" v-if="isAuth('jingsaibaoming','首页统计')"></div>
			<div id="chengjixinxiChart1" class="echarts3 animate__animated" v-if="isAuth('chengjixinxi','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getjingsaixinxiCount();
		this.getjingsaibaomingCount();
		if(this.isAuth('jingsaibaoming','首页统计')){
			this.jingsaibaomingChat1();
		}
		if(this.isAuth('jingsaibaoming','首页统计')){
			this.jingsaibaomingChat2();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'jingsaibaomingChart1',css:'animate__fadeInUp'},
				{id:'jingsaibaomingChart2',css:'animate__fadeInUp'},
		...
		getjingsaixinxiCount() {
			this.$http({
				url: `jingsaixinxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.jingsaixinxiCount = data.data
				}
			})
		},
		getjingsaibaomingCount() {
			this.$http({
				url: `jingsaibaoming/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.jingsaibaomingCount = data.data
				}
			})
		},
// 1234 竖
		jingsaibaomingChat1() {
			this.$nextTick(()=>{
				...
				var jingsaibaomingChart1 = echarts.init(document.getElementById("jingsaibaomingChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						jingsaibaomingChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							jingsaibaomingChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		jingsaibaomingChat2() {
			this.$nextTick(()=>{
				...
				var jingsaibaomingChart2 = echarts.init(document.getElementById("jingsaibaomingChart2"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						jingsaibaomingChart2.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							jingsaibaomingChart2.resize();
						};
					}else{
						this.$message({
							message: data.msg,
...
// 1234 折
		chengjixinxiChat1() {
			this.$nextTick(()=>{
				...
				var chengjixinxiChart1 = echarts.init(document.getElementById("chengjixinxiChart1"),'roma');
				this.$http({
...
<style lang="scss" scoped>
	.home-content {
		...
		margin: 0 auto;
		background: url(http://codegen.caihongy.cn/20250414/77ec6c5752b24f12bf88d3c2ea1d5fb2.png) no-repeat center top / cover;
		display: flex;
		width: calc(100% - 0px);
