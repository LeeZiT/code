	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="upload" v-if="type!='info'&& !ro.ziliaoxiazai" label="资料下载" prop="ziliaoxiazai" >
					<file-upload
						tip="点击上传资料下载"
						action="file/upload"
						:limit="1"
				...
				<el-form-item v-else-if="ruleForm.ziliaoxiazai" label="资料下载" prop="ziliaoxiazai" >
					<el-button class="downBtn" type="text" size="small" @click="download($base.url+ruleForm.ziliaoxiazai)">
			...
			<el-form-item v-else-if="ruleForm.ziliaojieshao" label="资料介绍" prop="ziliaojieshao" >
				<span class="text">{{ruleForm.ziliaojieshao}}</span>
			</el-form-item>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `ziyuanku/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.ziyuankuCrossAddOrUpdateFlag = false;
											this.parent.search();
