<template>
	<div class="main-content" :style='{"width":"calc(100% - 0px)","padding":"20px 30px 40px","margin":"0 auto","fontSize":"15px","background":"none"}'>
			...
			<el-form class="center-form-pv" :style='{"padding":"10px 0","margin":"0 0px 0px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"auto","padding":"0","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 0px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">竞赛名称</label>
						<el-input v-model="searchForm.jingsaimingcheng" placeholder="竞赛名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 0px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">竞赛类别</label>
						<el-input v-model="searchForm.jingsaileibie" placeholder="竞赛类别" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}' class="select" label="报名类型" prop="baomingleixing">
						<label :style='{"margin":"0 0px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">报名类型</label>
						<el-select clearable v-model="searchForm.baomingleixing" placeholder="请选择报名类型" >
				...
				<el-row class="actions" :style='{"width":"auto","padding":"0","margin":"0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('jingsaibaoming','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('jingsaibaoming','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('jingsaibaoming','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-shenhe9" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"34px"}'></span>
						审核
					</el-button>
					<el-button class="btn18" v-if="isAuth('jingsaibaoming','报名人数统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"34px"}'></span>
						报名人数统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('jingsaibaoming','竞赛热度统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"34px"}'></span>
						竞赛热度统计
					</el-button>
				</el-row>
			...
			<div :style='{"width":"100%","padding":"0 0px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="竞赛类别">
						<template slot-scope="scope">
							{{scope.row.jingsaileibie}}
						</template>
					</el-table-column>
					<el-table-column  :resizable='true' prop="jingsaifengmian" width="200" label="竞赛封面">
						<template slot-scope="scope">
							<div v-if="scope.row.jingsaifengmian">
								<img v-if="scope.row.jingsaifengmian.substring(0,4)=='http'&&scope.row.jingsaifengmian.split(',w').length>1" :src="scope.row.jingsaifengmian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.jingsaifengmian)">
								<img v-else-if="scope.row.jingsaifengmian.substring(0,4)=='http'" :src="scope.row.jingsaifengmian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.jingsaifengmian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.jingsaifengmian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.jingsaifengmian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' prop="baomingziliao" label="报名资料">
						<template slot-scope="scope">
							<el-button v-if="scope.row.baomingziliao" type="text" size="small" @click="download(scope.row.baomingziliao)">下载</el-button>
							<span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jingsaibaoming','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('jingsaibaoming','提交作品')" @click="zuopinxinxiCrossAddOrUpdateHandler(scope.row,'cross','是','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								提交作品
							</el-button>
							<el-button class="edit" v-if=" isAuth('jingsaibaoming','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('jingsaibaoming','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<zuopinxinxi-cross-add-or-update v-if="zuopinxinxiCrossAddOrUpdateFlag" :parent="this" ref="zuopinxinxiCrossaddOrUpdate"></zuopinxinxi-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="jingsaileibieChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
...
<script>
	...
	import zuopinxinxiCrossAddOrUpdate from "../zuopinxinxi/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				zuopinxinxiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			zuopinxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.zuopinxinxiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.zuopinxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.zuopinxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			chartDialog2() {
				...
				this.$nextTick(()=>{
					...
					var jingsaileibieChart2 = echarts.init(document.getElementById("jingsaileibieChart2"),'macarons');
					this.$http({
						url: "jingsaibaoming/group/jingsaileibie",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].jingsaileibie);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].jingsaileibie
								})
							}
							var option = {};
							let titleObj = this.pie.title
							titleObj.text = '竞赛热度统计'
							...
							// 使用刚指定的配置项和数据显示图表。
							jingsaileibieChart2.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								jingsaileibieChart2.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.jingsaileibie!='' && this.searchForm.jingsaileibie!=undefined){
					params['jingsaileibie'] = '%' + this.searchForm.jingsaileibie + '%'
				}
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "jingsaibaoming/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
